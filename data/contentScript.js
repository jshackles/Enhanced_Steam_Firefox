    	...
    	if (balance > 0) {
    		...
    		$(".btn_browse").each(function(index) {
    			if (index == 1) {
    				switch (currency_symbol) {
    					case "€":
    						$(this).after("<a class='btn_browse' style='width: 308px; background-image: url( http://www.enhancedsteam.com/firefox/es_btn_browse.png );' href='" + link + "'><h3 style='width: 120px;'>" + balance + "<span class='currency'>" + currency_symbol + "</span></h3><h5><span id='es_results'></span> games under " + balance_text + "</h5></a>");
    						break;
    					case "pуб":
    						$(this).after("<a class='btn_browse' style='width: 308px; background-image: url( http://www.enhancedsteam.com/firefox/es_btn_browse.png );' href='" + link + "'><h3 style='width: 120px;'>" + balance + "</h3><h5><span id='es_results'></span> games under " + balance_text + "</h5></a>");
    						break;
    					default:
    						$(this).after("<a class='btn_browse' style='width: 308px; background-image: url( http://www.enhancedsteam.com/firefox/es_btn_browse.png );' href='" + link + "'><h3 style='width: 120px;'><span class='currency'>" + currency_symbol + "</span>" + balance + "</h3><h5><span id='es_results'></span> games under " + balance_text + "</h5></a>");
    				}		
    ...
    if ($(".game_area_already_owned").length > 0) {
        ...
        get_http('https://store.steampowered.com/account/', function (txt) {
    		...
    		xpath_each("//div[contains(@class,'game_area_already_owned')]", function (node) {
    			if (found === 0) {
    				if (purchaseDate) {
    					$(node).append(localized_strings[language].purchase_date.replace("__date__", purchaseDate));
    					found = 1;
    				}
    			}
    		});
...
function add_steamcards_link(appid) {
    if ($(".icon").find('img[src$="/ico_cards.gif"]').length > 0) {
		$('.communitylink .block_content_inner:first').append("<a class='linkbar' href='http://steamcommunity.com/my/gamecards/" + appid + "'><div class='rightblock'><img src='http://www.enhancedsteam.com/firefox/ico_cards.gif' width='16' height='16' border='0' align='top' /></div>Steam Trading Cards</a>");
	}
}
				...
				if ($(this).attr("src").indexOf("http://cdn.steamcommunity.com/public/images/skin_1/ico_stats.gif") == 0) {
					if (!($(value).html().match(/<h5><\/h5>/))) {
						...
						$("#es_app_" + appid).load($(".profile_small_header_texture a")[0].href + '/stats/' + appid + ' #topSummaryAchievements', function(response, status, xhr) {							
							...
							html = html.replace(/achieveBarFull\.gif" border="0" height="12" width="([0-9]|[1-9][0-9]|[1-9][0-9][0-9])"/, "achieveBarFull.gif\" border='0' height='12' width=\"" + BarFull.toString() + "\"");
							html = html.replace(/achieveBarEmpty\.gif" border="0" height="12" width="([0-9]|[1-9][0-9]|[1-9][0-9][0-9])"/, "achieveBarEmpty.gif\" border='0' height='12' width=\"" + BarEmpty.toString() + "\"");
							html = html.replace("::", ":");
